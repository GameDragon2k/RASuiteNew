...
//	Exposed, shared
//	App-level:
bool    ( *_RA_GameIsActive) (void) = NULL;
void	( *_RA_CauseUnpause) (void) = NULL;
void	( *_RA_RebuildMenu) (void) = NULL;
void	( *_RA_ResetEmulation) (void) = NULL;
void	( *_RA_GetEstimatedGameTitle) (char* sNameOut) = NULL;
void	( *_RA_LoadROM) (char* sNameOut) = NULL;
...
int		(CCONV *_RA_InitI) ( HWND hMainWnd, int nConsoleID, const char* sClientVer, bool offline ) = NULL;
...
const char* CCONV _RA_InstallIntegration()
{
	...
	_RA_InitI				= (int(CCONV *)(HWND, int, const char*, bool))	GetProcAddress( g_hRADLL, "_RA_InitI" );;
...
void RA_Init( HWND hMainHWND, int nConsoleID, const char* sClientVersion )
{
	...
	if( bInstalled )
		_RA_InitI( hMainHWND, nConsoleID, sClientVersion, bOffline );
	else
		RA_Shutdown();
