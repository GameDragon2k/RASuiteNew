...
API BOOL CCONV _RA_InitI( HWND hMainHWND, /*enum EmulatorID*/int nEmulatorID, const char* sClientVer )
{
	...
	case RA_PSX:
		g_ConsoleID = 12; //
		g_sGetLatestClientPage = "LatestRAPCEVersion.html"; //doesn't actually exist
		g_sClientVersion = sClientVer;
		g_sClientName = "RA_PSX";
		g_sClientDownloadURL = "RA_PSX.zip";
		g_sClientEXEName = "RA_PSX.exe";
		break;

	case RA_ATARI:
		g_ConsoleID = 13; //
		g_sGetLatestClientPage = "LatestRAATARIVersion.html"; //doesn't actually exist
		g_sClientVersion = sClientVer;
		g_sClientName = "RA_ATARI";
		g_sClientDownloadURL = "RA_ATARI.zip";
		g_sClientEXEName = "RA_ATARI.exe";

		break;

	case RA_NEOGEO:
		g_ConsoleID = 14; //
		g_sGetLatestClientPage = "LatestRANEOGEOVersion.html"; //doesn't actually exist
		g_sClientVersion = sClientVer;
		g_sClientName = "RA_NEOGEO";
		g_sClientDownloadURL = "RA_NEOGEO.zip";
		g_sClientEXEName = "RA_NEOGEO.exe";
...
API HMENU CCONV _RA_CreatePopupMenu()
{
	...
	if( g_LocalUser.m_bIsLoggedIn )
	{
		...
		AppendMenu( hRA, g_bLeaderboardsActive ? MF_CHECKED : MF_UNCHECKED, IDM_RA_TOGGLELEADERBOARDS, TEXT("Toggle &Leaderboards") );
	}
	else
	{
...
API void CCONV _RA_LoadPreferences()
{
	...
	if( pConfigFile == NULL )
	{
		...
		sprintf_s( sWelcomeMessage, 4096, 
			...
			"Quick Start: Press ESCAPE or 'Back' on your emulator to view the achievement overlay.\n\n" );
