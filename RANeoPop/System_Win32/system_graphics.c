...
void system_graphics_update_window(void)
{
	...
	input.m_bUpPressed = up;
	input.m_bDownPressed = down;
	input.m_bLeftPressed = left;
	input.m_bRightPressed = right;
	input.m_bCancelPressed = button_b;
	input.m_bConfirmPressed = button_a;
	input.m_bQuitPressed = option;

	RenderAchievementsOverlay(g_hWnd, input, system_paused);
