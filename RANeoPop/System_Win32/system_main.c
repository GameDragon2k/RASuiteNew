...
static void menu_SetEnabled(bool enabled) {
	if (enabled) {
		SetMenu(g_hWnd, g_menu);
...
static LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		//	##RA
		if (LOWORD(wParam) >= IDM_RA_RETROACHIEVEMENTS &&
...
BOOL WinInit(void)
{
	...
	AppendMenu(hRA, MF_SEPARATOR, NULL, NULL);
	AppendMenu(hRA, MF_UNCHECKED, IDM_RA_HARDCORE_MODE, TEXT("&Hardcore Mode"));
	AppendMenu(hRA, MF_UNCHECKED, IDM_RA_TOGGLELEADERBOARDS, TEXT("&Toggle Leaderboards"));
	AppendMenu(hRA, MF_SEPARATOR, NULL, NULL);
	...
	//	##RA embed RA
	AppendMenu(g_menu, MF_POPUP | MF_STRING, (UINT_PTR)hRA, TEXT("&RetroAchievements"));
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE a, LPSTR cmdline, int c)
{
	...
	menu_SetEnabled(menu_enabled);

	//=========

	RA_Init(g_hWnd, RA_NEOGEO, "0.020");
	RA_UpdateAppTitle("");
	...
	do
	{
		...
		//======================================
		if (system_paused == TRUE || (auto_pause && misc_autopause))
		{
